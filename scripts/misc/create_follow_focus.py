...
def create_follow_focus():
    ...
    if sel_nodes:
        ...
        distance_node_loc_focal = cmds.rename(distance_node_loc_focal,
                                              sel_nodes[0] + "FfFocalPt_LOC")
        ...
        cmds.aimConstraint(
            sel_nodes[0], distance_node_loc_focal, maintainOffset=False)

        # Lock and hide translation and rotation for cam locator
        cmds.setAttr(
            distance_node_loc_cam + '.tx',
            lock=True,
            keyable=False,
            channelBox=False)
        cmds.setAttr(
            distance_node_loc_cam + '.ty',
            lock=True,
            keyable=False,
            channelBox=False)
        cmds.setAttr(
            distance_node_loc_cam + '.tz',
            lock=True,
            keyable=False,
            channelBox=False)
        cmds.setAttr(
            distance_node_loc_cam + '.rx',
            lock=True,
            keyable=False,
            channelBox=False)
        cmds.setAttr(
            distance_node_loc_cam + '.ry',
            lock=True,
            keyable=False,
            channelBox=False)
        cmds.setAttr(
            distance_node_loc_cam + '.rz',
            lock=True,
            keyable=False,
            channelBox=False)
